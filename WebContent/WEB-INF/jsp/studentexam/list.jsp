...
function show(id){
	//windowOpen('${ctx }/examController/startExam/'+id+".do")
	window.location.href = '${ctx }/examController/startExam/'+id+".do";
}
								...
								<tbody>
									<c:forEach items="${examList}" var="exam">
										<tr>
											...
											<td>${exam.userName }</td>
											<td>
											<c:if test="${exam.state eq 0}">
													<Label>未到考试时间</Label>
												</c:if>
												<c:if test="${exam.state eq 1}">
													<Label>可以考试</Label>
												</c:if>
												<c:if test="${exam.state eq 2}">
													<Label>已考</Label>
												</c:if>
												<c:if test="${exam.state eq 3}">
													<Label>考试时间已过</Label>
												</c:if>
											</td>
											<td>
													<c:if test="${exam.state eq 1}">
													<a class="btn btn-xs btn-info" onclick="show('${exam.examId}')" title="开始考试">
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											<a href="<%=request.getContextPath() %>/examController/studentExam.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
