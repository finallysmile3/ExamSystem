...
function windowOpen(url)
{
	...
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("get:"+url, { title: "题目详解",width: iWidth,height: iHeight,submit: myfunc ,
         buttons: { '关闭': false }}); 
}
function windowOpen1(url)
{
	...
	var iTop = (window.screen.availHeight-30-iHeight)/2; //获得窗口的垂直位置;
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("iframe:"+url, { title: "题目详解",width: iWidth,height: iHeight,submit: myfunc ,
         buttons: {}}); 
}
function show(id){
	windowOpen('${ctx }/pointController/show/'+id+".do")
}
...
var myfunc = function (v, h, f) {
    ...
    if (v== true ){    //选择清空时，返回false.然后清空。
    	...
    	var url = "<%=request.getContextPath() %>/choiceErrorController/all.do";
...
var submit1 = function (v, h, f) {
    if (v == 'ok')
    {
	   	 var url = '${ctx }/choiceErrorController/delete/'+cartypeid+".do";
	   	 xmlhttp.open("GET", url, true);
...
function addQuestion(){
	windowOpen('${ctx }/choiceErrorController/addOrUpdate/'+0+'.do')
}
function addQuestion1(){
	windowOpen('${ctx }/choiceErrorController/addOrUpdate1/'+0+'.do')
}
function download(){
...
$(function(){
   	...
   	$("#searchButton").click(function(){
	   	...
	   	choiceQuestion.subjectId=subjectIdvalue;
   	   	...
   	   	var url = "<%=request.getContextPath() %>/choiceErrorController/search.do?";
						...
						<div class="col-xs-12">
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								<thead>
									<tr>	
										<th>题目名称</th>
										<th style="width: 60px">题目类别</th>
										<th>选项A</th>
										<th>选项B</th>
										<th>选项C</th>
										<th>选项D</th>																
										<th>正确答案</th>
										<th style="width: 60px">涉及知识点</th>
								...
								<tbody>								
									<c:forEach items="${choiceErrorList.datas}" var="choiceQuestion">
										<tr>
											...
											<td>${choiceQuestion.questionOptionA}</td>	
											<td>${choiceQuestion.questionOptionB }</td>
											<td>${choiceQuestion.questionOptionC }</td>
											<td>${choiceQuestion.questionOptionD }</td>
											<td>${choiceQuestion.correctOption}</td>
											<td>${choiceQuestion.pointName}</td>
											<td>${choiceQuestion.subjectName}</td>
											<td>
													<a class="btn btn-xs btn-info" onclick="show('${choiceQuestion.pointId}')" title="查看知识点详解">
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											<a href="<%=request.getContextPath() %>/choiceErrorController/all.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
<!-- 											<a class="btn btn-info fa" style="margin-left: 30px" onclick="forward()" title="跳转部门页面">查看部门信息</a> -->
											</td>
											<td style="vertical-align: top;">
												 <c:if test="${choiceErrorList.total > 0}">
													<jsp:include page="/pager.jsp">
														<jsp:param value="${choiceErrorList.total }" name="totalRecord"/>
