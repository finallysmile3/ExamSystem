...
function windowOpen(url)
{
	...
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("get:"+url, { title: "科目",width: iWidth,height: iHeight,submit: myfunc ,
...
function windowOpen1(url)
{
	...
	var iTop = (window.screen.availHeight-30-iHeight)/2; //获得窗口的垂直位置;
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("iframe:"+url, { title: "科目",width: iWidth,height: iHeight,submit: myfunc ,
         buttons: {}}); 
}
function show(id){
	windowOpen('${ctx }/subjectController/addOrUpdate/'+id+".do")
}
...
var myfunc = function (v, h, f) {
    ...
    if (v== true ){    //选择清空时，返回false.然后清空。
    	...
    	var url = "<%=request.getContextPath() %>/subjectController/all.do";
...
var submit1 = function (v, h, f) {
    if (v == 'ok')
    {
	   	 var url = '${ctx }/subjectController/delete/'+cartypeid+".do";
	   	 xmlhttp.open("GET", url, true);
...
function del(carTypeId){
	cartypeid = carTypeId;
	jBox.confirm("确定吗？", "确认删除？将删除科目的相关信息", submit1);
...
function addQuestion(){
	windowOpen('${ctx }/subjectController/addOrUpdate/'+0+'.do')
}
function download(){
...
$(function(){
   	...
   	$("#searchButton").click(function(){
   	   	var subjectrNameValue = $("#subjectName").val();
   		var subjectStateValue = $("#subjectState").val();
   	   	var user= new Object();
	   	user.subjectName=subjectrNameValue;
	   	user.subjectState=subjectStateValue;
   	 	
   	   	var url = "<%=request.getContextPath() %>/subjectController/search.do?";
...
<body class="no-skin" >
	<div class="main-content-inner">
		...
		<div class="page-content">
				科目名称：<input type="text" name="subjectName" id="subjectName" value="${subjectName}"/>&nbsp;&nbsp;
				科目状态:<select id="subjectState" name="subjectState" style="140px">
							<option value="">--选择状态--</option>
							<option value="0" <c:if test="${subjectState eq 0}">selected="selected"</c:if>>可用</option>
							<option value="1" <c:if test="${subjectState eq 1}">selected="selected"</c:if>>不可用</option>
						</select>
					...
					<div class="row">
						<div class="col-xs-12">
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								<thead>
									<tr>
										<th style="width: 81px">科目名称</th>
										<th style="width: 70px">科目状态</th>
										<th>科目描述</th>
								...
								<tbody>								
									<c:forEach items="${subjectList.datas}" var="subject">
										<tr>
											...
											<td>																							
													<a class="btn btn-xs btn-info" onclick="show('${subject.subjectId}')" title="编辑">
															<i class="ace-icon fa fa-pencil bigger-120"></i>
													</a>
												
													<a class="btn btn-xs btn-danger" onclick="del('${subject.subjectId }')" title="删除">
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											<a class="btn btn-info fa" onclick="addQuestion()" title="添加科目
											">添加科目</a>
											<a href="<%=request.getContextPath() %>/subjectController/all.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
<!-- 											<a class="btn btn-info fa" style="margin-left: 30px" onclick="forward()" title="跳转部门页面">查看部门信息</a> -->
											</td>
											<td style="vertical-align: top;">
												 <c:if test="${subjectList.total > 0}">
													<jsp:include page="/pager.jsp">
														<jsp:param value="${subjectList.total }" name="totalRecord"/>
