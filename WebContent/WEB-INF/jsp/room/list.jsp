...
function windowOpen(url)
{
	...
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("get:"+url, { title: "房间",width: iWidth,height: iHeight,submit: myfunc ,
...
function windowOpen1(url)
{
	...
	var iTop = (window.screen.availHeight-30-iHeight)/2; //获得窗口的垂直位置;
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("iframe:"+url, { title: "房间",width: iWidth,height: iHeight,submit: myfunc ,
         buttons: {}}); 
}
function show(id){
	windowOpen('${ctx }/roomController/addOrUpdate/'+id+".do")
}
...
var myfunc = function (v, h, f) {
    ...
    if (v== true ){    //选择清空时，返回false.然后清空。
    	...
    	var url = "<%=request.getContextPath() %>/roomController/all.do";
...
var submit1 = function (v, h, f) {
    if (v == 'ok')
    {
	   	 var url = '${ctx }/roomController/delete/'+cartypeid+".do";
	   	 xmlhttp.open("GET", url, true);
...
function addQuestion(){
	windowOpen('${ctx }/roomController/addOrUpdate/'+0+'.do')
}
function download(){
...
$(function(){
   	...
   	$("#searchButton").click(function(){
   	   	var roomNameValue = $("#roomName").val();
   	   	var roomStateValue = $("#roomState").val();
   	   	var room= new Object();
	   	room.roomName=roomNameValue;
	   	room.roomState=roomStateValue;
   	 	
   	   	var url = "<%=request.getContextPath() %>/roomController/search.do?";
   	 	var json=stringify(room);
...
<body class="no-skin" >
	<div class="main-content-inner">
		...
		<div class="page-content">
				教室名称：<input type="text" name="roomName" id="roomName" value="${roomName}"/>&nbsp;&nbsp;
				教室状态:<select id="roomState" name="roomState" style="140px">
							<option value="">--选择状态--</option>
							<option value="0" <c:if test="${roomState eq 0}">selected="selected"</c:if>>可用</option>
							<option value="1" <c:if test="${roomState eq 1}">selected="selected"</c:if>>不可用</option>
						</select>
					...
					<div class="row">
						<div class="col-xs-12">
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<tbody>								
									<c:forEach items="${roomList.datas}" var="room">
										<tr>
											<td>${room.roomName}</td>
											<td>
											<c:if test="${room.roomState eq 0}">
													<Label>可用</Label>
												</c:if>
												<c:if test="${room.roomState eq 1}">
													<Label>不可用</Label>
												</c:if>
											</td>
											<td>
													<a class="btn btn-xs btn-info" onclick="show('${room.roomId}')" title="编辑">
															<i class="ace-icon fa fa-pencil bigger-120"></i>
													</a>
												
													<a class="btn btn-xs btn-danger" onclick="del('${room.roomId }')" title="删除">
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											<a class="btn btn-info fa" onclick="addQuestion()" title="添加教室
											">添加教室</a>
											<a href="<%=request.getContextPath() %>/roomController/all.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
<!-- 											<a class="btn btn-info fa" style="margin-left: 30px" onclick="forward()" title="跳转部门页面">查看部门信息</a> -->
											</td>
											<td style="vertical-align: top;">
												 <c:if test="${roomList.total > 0}">
													<jsp:include page="/pager.jsp">
														<jsp:param value="${roomList.total }" name="totalRecord"/>
