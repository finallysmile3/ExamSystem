...
function show(id){
	windowOpen('${ctx }/judgeQuestionController/addOrUpdate/'+id+".do")
}
...
var myfunc = function (v, h, f) {
    ...
    if (v== true ){    //选择清空时，返回false.然后清空。
    	...
    	var url = "<%=request.getContextPath() %>/judgeQuestionController/all.do";
...
var submit1 = function (v, h, f) {
    if (v == 'ok')
    {
	   	 var url = '${ctx }/judgeQuestionController/delete/'+cartypeid+".do";
	   	 xmlhttp.open("GET", url, true);
...
function addQuestion(){
	windowOpen('${ctx }/judgeQuestionController/addOrUpdate/'+0+'.do')
}
function download(){
...
$(function(){
   	...
   	$("#searchButton").click(function(){
	   	...
	   	essayQuestion.subjectId=essayQuestionSexValue;
   	   	...
   	   	var url = "<%=request.getContextPath() %>/judgeQuestionController/search.do?";
					...
					<div class="row">
						<div class="col-xs-12">
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<tbody>								
									<c:forEach items="${judgeQuestionList.datas}" var="judgeQuestion">
										<tr>
											<td>${judgeQuestion.questionName}</td>	
											<td>
											<c:if test="${judgeQuestion.correctAnswer eq 'T'}">
													<Label>√</Label>
												</c:if>
												<c:if test="${judgeQuestion.correctAnswer eq 'F'}">
													<Label>×</Label>
												</c:if>
											</td>
											<td>${judgeQuestion.pointName }</td>
											<td>${judgeQuestion.subjectName}</td>
											<td>																							
													<a class="btn btn-xs btn-info" onclick="show('${judgeQuestion.questionId}')" title="编辑">
															<i class="ace-icon fa fa-pencil bigger-120"></i>
													</a>
												
													<a class="btn btn-xs btn-danger" onclick="del('${judgeQuestion.questionId }')" title="删除">
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											">添加题目</a>
											<a href="<%=request.getContextPath() %>/judgeQuestionController/all.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
<!-- 											<a class="btn btn-info fa" style="margin-left: 30px" onclick="forward()" title="跳转部门页面">查看部门信息</a> -->
											</td>
											<td style="vertical-align: top;">
												 <c:if test="${judgeQuestionList.total > 0}">
													<jsp:include page="/pager.jsp">
														<jsp:param value="${judgeQuestionList.total }" name="totalRecord"/>
