...
function addQuestion(){
	windowOpen('${ctx }/scoreController/addOrUpdate/'+0+'.do')
}
function download(){
...
$(function(){
   	...
   	$("#searchButton").click(function(){
	   	...
	   	score.classId=scoreNameValue;
   	   	...
   	   	var url = "<%=request.getContextPath() %>/scoreController/search1.do?";
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											<a href="<%=request.getContextPath() %>/scoreController/readed.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
											<a  class="btn btn-info fa" style="margin-left: 20px"
												href="<%=request.getContextPath()%>/scoreController/download.do"
												title="导出分数到Excel" class="btn btn-info fa fa-refresh">导出分数统计表</a>
											</td>
											<td style="vertical-align: top;">
												 <c:if test="${scoreList.total > 0}">
													<jsp:include page="/pager.jsp">
														<jsp:param value="${scoreList.total }" name="totalRecord"/>
														<jsp:param value="search1.do" name="url"/>
													</jsp:include>
												</c:if> 
											</td>
										</tr>
