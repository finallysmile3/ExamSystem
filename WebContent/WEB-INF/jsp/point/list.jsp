...
function windowOpen(url)
{
	...
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("get:"+url, { title: "知识点",width: iWidth,height: iHeight,submit: myfunc ,
...
function windowOpen1(url)
{
	...
	var iTop = (window.screen.availHeight-30-iHeight)/2; //获得窗口的垂直位置;
	var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	 jBox("iframe:"+url, { title: "知识点",width: iWidth,height: iHeight,submit: myfunc ,
         buttons: {}}); 
}
function show(id){
	windowOpen('${ctx }/pointController/addOrUpdate/'+id+".do")
}
...
var myfunc = function (v, h, f) {
    ...
    if (v== true ){    //选择清空时，返回false.然后清空。
    	...
    	var url = "<%=request.getContextPath() %>/pointController/all.do";
...
var submit1 = function (v, h, f) {
    if (v == 'ok')
    {
	   	 var url = '${ctx }/pointController/delete/'+cartypeid+".do";
	   	 xmlhttp.open("GET", url, true);
...
function del(carTypeId){
	cartypeid = carTypeId;
	jBox.confirm("确定吗？", "确认删除？将删除知识点的相关信息", submit1);
...
function addQuestion(){
	windowOpen('${ctx }/pointController/addOrUpdate/'+0+'.do')
}
function download(){
...
$(function(){
   	...
   	$("#searchButton").click(function(){
   	   	var pointNoValue = $("#pointNo").val();
   	   	var pointNameValue = $("#pointName").val();
   		var pointSexValue = $("#pointSex").val();
   		var pointTypeValue = $("#pointType").val();
   		var pointStateValue = $("#pointState").val();
   	   	var point= new Object();
	   	 point.loginName=pointNoValue;
	   	 point.pointName=pointNameValue;
	   	 point.pointSex=pointSexValue;
	   	 point.pointType=pointTypeValue;
	   	 point.pointState=pointStateValue;
   	 	
   	   	var url = "<%=request.getContextPath() %>/pointController/search.do?";
   	 	var json=stringify(point);
...
<body class="no-skin" >
	<div class="main-content-inner">
		...
		<div class="page-content">
				知识点名称：<input type="text" name="pointName" id="pointName" value="${pointName}"/>&nbsp;&nbsp;
				所属科目:<select id="subjectId" name="subjectId" style="140px">
					...
					<div class="row">
						<div class="col-xs-12">
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<tbody>								
									<c:forEach items="${pointList.datas}" var="point">
										<tr>
											<td>${point.pointName}</td>
											<td>${point.pointContent}</td>
											<td>${point.subjectName}</td>
											<td>																							
													<a class="btn btn-xs btn-info" onclick="show('${point.pointId}')" title="编辑">
															<i class="ace-icon fa fa-pencil bigger-120"></i>
													</a>
												
													<a class="btn btn-xs btn-danger" onclick="del('${point.pointId }')" title="删除">
								...
								<table style="width: 100%;">
									<tbody>
										<tr>
											...
											<a class="btn btn-info fa" onclick="addQuestion()" title="添加知识点
											">添加知识点</a>
											<a href="<%=request.getContextPath() %>/pointController/all.do" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
<!-- 											<a class="btn btn-info fa" style="margin-left: 30px" onclick="forward()" title="跳转部门页面">查看部门信息</a> -->
											</td>
											<td style="vertical-align: top;">
												 <c:if test="${pointList.total > 0}">
													<jsp:include page="/pager.jsp">
														<jsp:param value="${pointList.total }" name="totalRecord"/>
